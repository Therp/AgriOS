...
class PurchaseOrder(models.Model):
    ...
    oms_oa_id = fields.Many2one('offtake.agreement', 'OMS Contract', domain="[('outgrower_id','=',partner_id),('contract_stage','=','open'),('company_id','=',company_id)]")
    ...
    def _onchange_oms_contract(self):
        if self.partner_id:
            if self.oms_oa_id and self.oms_oa_id.outgrower_id == self.partner_id:
                return
            
            company = self.company_id or self.env.company
            
            if company.oms_allow_operations_out_of_phase:
                contracts = self.partner_open_contract_ids
            else:
                contracts = self.partner_open_contract_harvest_ids
            
            if len(contracts) == 1:
                self.oms_oa_id = contracts
            else:
                self.oms_oa_id = False
        else:
            self.oms_oa_id = False
    
    @api.onchange('oms_oa_id')
    def _onchange_oms_oa_id(self):
        if self.oms_oa_id:
            oftake_prd = self.oms_oa_id.contracted_crop_id
            balance_qty = max(self.oms_oa_id.balance_qty * -1, 0)
            ...
            self.currency_id = self.oms_oa_id.currency_id
    

class PurchaseOrderLine(models.Model):
    _inherit = 'purchase.order.line'
    
    oms_oa_line_id = fields.Many2one('offtake.agreement', related='order_id.oms_oa_id')
    ...
    region_id = fields.Many2one(related='partner_id.coop_id.district_id.region_id', store=True)
    ...
    def _compute_price_unit_and_date_planned_and_name(self):
        ...
        if self.order_id.oms_oa_id and self.product_id and self.product_id == self.order_id.oms_oa_id.contracted_crop_id:
            self.price_unit = self.order_id.oms_oa_id.offtake_unit_price
