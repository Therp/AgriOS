...
class Farmer(models.Model):
    ...
    def _default_farm_uom(self):
        ...
        if not farm_uom:
            farm_uom = self.env.ref('ai_oms.area_1', raise_if_not_found=False)
        return farm_uom
    ...
    oms_unreconciled_aml_ids = fields.One2many('account.move.line', compute='_compute_oms_unreconciled_aml_ids', readonly=False) # copy of enterprise unreconciled_aml_ids
    
    @api.depends('invoice_ids')
    @api.depends_context('company', 'allowed_company_ids')
    def _compute_oms_unreconciled_aml_ids(self):
        if not self: return
        
        if hasattr(self[0], 'unreconciled_aml_ids'):
            for ptn in self:
                ptn.oms_unreconciled_aml_ids = ptn.unreconciled_aml_ids
            ...
            for partner in self:
                partner.oms_unreconciled_aml_ids = self.env['account.move.line'].browse(unreconciled_aml_ids.get(partner, []))
        ...
        for farmer in self:
            if farmer.outgrower_stage == 'verified':
                if farmer.open_contract_input_ids:
                    ...
                    farmer.can_order_inputs_confirm = (farmer.company_id or self.env.company).oms_allow_operations_out_of_phase
        ...
        for farmer in self:
            if farmer.outgrower_stage == 'verified':
                if farmer.open_contract_harvest_ids:
                    ...
                    farmer.can_regist_offtakes_confirm = (farmer.company_id or self.env.company).oms_allow_operations_out_of_phase
    ...
    def _get_view(self, view_id=None, view_type='form', **options):
        ...
        if view_type == 'search' and view == self.env.ref('ai_oms.view_outgrower_search_filter', raise_if_not_found=False):
        ...
        elif view_type == 'list' and view == self.env.ref('ai_oms.view_outgrower_tree', raise_if_not_found=False):
            company_country_id = self.env.company.country_id.id
            loc_details, max_level = self.env['country.location.level']._get_country_details(company_country_id)
    ...
    def verify_outgrower(self):
        for outgrower in self:
            vals = {
                ...
                'outgrower_requires_contract': (outgrower.company_id or self.env.company).oms_default_contract_needed,
            }
    ...
    def action_view_oms_sale_orders(self):
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
    ...
    def action_view_oms_purchase_orders(self):
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
    ...
    def action_create_oms_so(self):
        ...
        if not self.outgrower_requires_contract and self.harvest_crop_ids:
            oms_allow_operations_out_of_phase = (self.company_id or self.env.company).oms_allow_operations_out_of_phase
            if not (self.open_contract_input_ids or (oms_allow_operations_out_of_phase and self.open_contract_ids)):
