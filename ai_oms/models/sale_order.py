...
class SaleOrder(models.Model):
    ...
    def _onchange_oms_oa_id(self):
        if self.oms_oa_id:
            ...
            seed_prd = self.oms_oa_id.seed_variety_id
            if seed_prd:
                input_products |= seed_prd
                input_products |= seed_prd.related_inputs_ids
            input_products |= self.oms_oa_id.contracted_crop_id.related_inputs_ids
            
            for input_prd in input_products:
                vals = {
                    'product_id': input_prd.id,
                    'product_uom_qty': (self.oms_oa_id.contracted_farm_acreage * input_prd.qty_per_acreage) or 1.0,
                }
...
class SaleOrderLine(models.Model):
    ...
    oms_oa_line_id = fields.Many2one(related='order_id.oms_oa_id')
    ...
    def _onchange_oms_order(self):
        for order in self:
            if order.oms_oa_line_id:
                order.product_uom_qty = max(order.product_template_id.qty_per_acreage * order.oms_oa_line_id.contracted_farm_acreage, 1)
