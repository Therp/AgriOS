...
class PlantAgeYield(models.Model):
    ...
    crop_product_id = fields.Many2one('product.template', string='Crop Product', domain=[('crop_product', '=', True),('harvest_product_type','=','tree_crop')], required=True, ondelete='cascade', index=True)
    ...
    _sql_constraints = [
        ('unique_product_age', 'UNIQUE(crop_product_id, age)', 'A yield record for this crop and age already exists.'),
