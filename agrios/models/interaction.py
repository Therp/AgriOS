...
class FarmerInteraction(models.Model):
    ...
    farmer_id = fields.Many2one('res.partner', 'Farmer', domain=[('is_farmer','=',True)], tracking=True, index=True)
    contract_id = fields.Many2one('farmer.contract', domain="[('farmer_id','=',farmer_id)]", tracking=True)
