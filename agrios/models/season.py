    ...
    def unlink(self):
        for season in self:
            if season.status != 'draft':
                raise UserError(_("Only seasons in draft status can be deleted! Archive the season instead."))
        return super().unlink()
    
    def _compute_expired(self):
        for season in self:
            season.expired = (season.status in ('open', 'lock') and season.end_date < fields.Date.today())
