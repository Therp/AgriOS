...
class SaleOrder(models.Model):
    ...
    partner_open_contract_input_ids = fields.One2many(
        related="partner_id.open_contract_input_ids", readonly=True
    )
    ...
    def _onchange_agrios_oa_id(self):
        if self.farmer_contract_id:
            ...
            input_products |= (
                self.farmer_contract_id.contracted_crop_id.related_inputs_ids
            )
            ...
            for input_prd in input_products:
                vals = {
                    ...
                    "product_uom_qty": (
                        self.farmer_contract_id.contract_acreage
                        * input_prd.qty_per_acreage
                    )
                    or 1.0,
                }
        ...
        if self.payment_in_kind:
            ...
            if not payment_in_kind_journals:
                raise ValidationError(
                    _("No journals defined as a 'payment in kind' journal")
                )
