...
class FarmerContract(models.Model):
    ...
    contracted_crop_id = fields.Many2one(
        "product.product", required=True, domain="[('id','in',farmer_crop_product_ids)]"
    )

    available_seed_ids = fields.One2many(
    ...
    total_contracted_acreage = fields.Float(
        related="farmer_id.total_contracted_acreage"
    )
    non_contracted_acreage = fields.Float(related="farmer_id.non_contracted_acreage")

    contracted_offtake_qty = fields.Integer(tracking=True)
    received_offtake_qty = fields.Integer(
        "Received Qty", compute="_compute_received_offtake_qty"
    )
    offtake_available_qty = fields.Integer(
        "Pending Offtake Qty", compute="_compute_offtake_available_qty"
    )

    contracted_unit_price = fields.Monetary()
    total_contract_cost = fields.Monetary(compute="_compute_total_offtake_cost")
    total_inputs_cost = fields.Monetary(compute="_compute_total_inputs_cost")

    oa_input_ids = fields.One2many(
        "sale.order.line",
        "farmer_contract_line_id",
    ...
    farmer_contract_offtake_ids = fields.One2many(
        "purchase.order.line",
        "farmer_contract_line_id",
        string="Farmer Contract Offtakes",
        domain=[("state", "not in", ["draft", "sent", "to approve", "cancel"])],
    )

    interaction_ids = fields.One2many("farmer.interaction", "contract_id")
    ready_for_harvest = fields.Boolean(default=False, copy=False)
    harvest_state = fields.Selection(
        selection=[
            ("input", "Input Stage"),
            ("harvest", "Harvest Stage"),
        ],
        string="Input/Harvest Stage",
        compute="_compute_harvest_state",
        store=True,
    )

    balance_qty = fields.Integer(compute="_compute_balance_qty")
    same_unutilized_contracted = fields.Boolean(
        compute="_compute_same_unutilized_contracted",
        help="If the contracted area is the same as the unutilized area",
    )

    display_name = fields.Char(compute="_compute_display_name", store=True)

    active_contract = fields.Boolean(compute="_compute_active_contract")
    expired_contract = fields.Boolean(
        "Contract Expired",
        compute="_compute_expired_contract",
        search="_search_expired_contract",
    )

    has_offtake_order = fields.Boolean(
        compute="_compute_has_offtake_order",
        search="_search_has_offtake_order",
    )
    ...
    farmer_crop_product_ids = fields.Many2many(
        related="farmer_id.crop_product_ids", string="Crop Products"
    )
    ...
    def _compute_harvest_state(self):
        for contract in self:
            if contract.contract_stage == "open":
                contract.harvest_state = (
                    contract.ready_for_harvest and "harvest" or "input"
                )
            else:
    ...
    def _compute_can_order_inputs(self):
        for contract in self:
            contract.can_order_inputs = bool(
                contract.contract_stage == "open"
                and (
                    not contract.ready_for_harvest
                    or contract.company_id.allow_operations_out_of_phase
                )
            )
    ...
    def _compute_can_register_offtakes(self):
        for contract in self:
            contract.can_register_offtakes = bool(
                contract.contract_stage == "open"
                and (
                    contract.ready_for_harvest
                    or contract.company_id.allow_operations_out_of_phase
                )
            )
    ...
    def _compute_has_offtake_order(self):
        for contract in self:
            contract.has_offtake_order = bool(
                contract.farmer_contract_offtake_ids.filtered(
    ...
    def _compute_offtake_available_qty(self):
        for contract in self:
            if contract.contract_stage == "open":
                contract.offtake_available_qty = (
                    contract.contracted_offtake_qty - contract.received_offtake_qty
                )
            else:
    ...
    def _compute_active_contract(self):
        ...
        for contract in self:
            if (
                contract.contract_stage != "open"
                or today > contract.season_end_date
                or today < contract.season_start_date
            ):
    ...
    def _compute_same_unutilized_contracted(self):
        for offtake_contract in self:
            offtake_contract.same_unutilized_contracted = (
                offtake_contract.non_contracted_acreage
                == offtake_contract.contract_acreage
            )

    def _compute_count_oa_input_sales(self):
        for rec in self:
            rec.count_oa_input_sales = self.env["sale.order"].search_count(
                [("farmer_contract_id", "=", rec.id)]
            )

    def _compute_count_oa_purchases(self):
        for rec in self:
            rec.count_oa_purchases = self.env["purchase.order"].search_count(
                [("farmer_contract_id", "=", rec.id)]
            )
    ...
    def _compute_total_offtake_cost(self):
        for rec in self:
            rec.total_contract_cost = (
                rec.contracted_offtake_qty * rec.contracted_unit_price
            )
    ...
    def _compute_display_name(self):
        for rec in self:
            rec.display_name = (
                f"[{rec.name}] {rec.farmer_id.name} {rec.season_id.season}"
                f" {rec.contracted_crop_id.name}"
                if rec.name != "/"
                else f"{rec.farmer_id.name} {rec.season_id.season}"
                f" {rec.contracted_crop_id.name}"
            )
    ...
    def _search_expired_contract(self, operator, value):
        ...
        self._cr.execute("""
            ...
            INNER JOIN season ON cont.season_id = season.id
            AND season.end_date < CURRENT_DATE
    ...
    def _search_has_offtake_order(self, operator, value):
        ...
        self._cr.execute("""
            ...
            INNER JOIN purchase_order po
               ON po.farmer_contract_id = cont.id AND po.state != 'cancel'
            """)
        return [("id", domain_operator, [r[0] for r in self._cr.fetchall()])]
    ...
    def _onchange_offtake_qty(self):
        for rec in self:
            if rec.contracted_crop_id and rec.contract_acreage:
                ...
                est_offtake = (
                    crop_product.estimated_yield
                    if crop_product.harvest_product_type == "perennial"
    ...
    def _onchange_contracted_crop_id(self):
        if self.contracted_crop_id:
            if len(self.available_seed_ids) == 1:
                ...
                self.seed_variety_id
                and self.seed_variety_id.crop_product_id != self.contracted_crop_id
            ):
    ...
    def _onchange_farmer_id(self):
        if self.farmer_id:
            ...
            if (
                self.contracted_crop_id
                and self.contracted_crop_id not in self.farmer_crop_product_ids
            ):
    ...
    def unlink(self):
        for contract in self:
            if contract.name != "/":
                raise UserError(
                    _(
                        "You can't delete a contract with a reference generated!"
    ...
    def name_search(self, name="", args=None, operator="ilike", limit=100):
        if self._context.get("verify_agrios_purchase"):
            ...
            company = (
                ...
                else self.env["res.company"].browse(company_id)
            )
    ...
    def btn_set_contracted_mapped_qty(self):
        for offtake_contract in self:
            if offtake_contract.contract_stage != "draft":
                raise UserError(
                    _(
                        "The contracted area can only be changed"
                        " when the contract is in draft status."
                    )
                )
    ...
    def action_agrios_oa_sale_orders(self):
        ...
        return {
            ...
            "help": (
                ...
                "Contract without any order inputs</p>"
            ),
            "context": {
                "create": False,
            },
    ...
    def action_agrios_oa_purchase_orders(self):
        ...
        return {
            ...
            "help": (
                '<p class="o_view_nocontent_smiling_face">'
                "Contract without any off-takes</p>"
            ),
            "context": {
                "create": False,
            },
    ...
    def confirm_farmer_contract(self):
        farm_area_cache = {}
        # First do the validation loop to not create gaps in the sequence
        # in case some case fails.
        for offtake_contract in self:
            if offtake_contract.contracted_offtake_qty <= 0.0:
                raise ValidationError(
                    _(
                        f"Contract {offtake_contract.display_name}"
                        f" Warning - Offtake quantity must be greater than zero"
                    )
                )
            ...
            if offtake_contract.contract_acreage <= 0.0:
                raise ValidationError(
                    _(
                        f"Contract {offtake_contract.display_name}"
                        f" Warning - Contracted acreage must be greater than zero"
                    )
                )

            if offtake_contract.farmer_id.id not in farm_area_cache:
                farm_area_cache[offtake_contract.farmer_id.id] = (
                    offtake_contract.non_contracted_acreage
                )
            ...
            if (
                ...
                raise ValidationError(
                    _(
                        f"Contract {offtake_contract.display_name} Warning"
                        " - Contracted acreage cannot be higher than the current"
                        " unutilized farm value. If more acreage is to be"
                        " contractualized for this farmer, the plots need to be"
                        " mapped first, and added to the system"
                    )
                )
    ...
    def cancel_farmer_contract(self):
        for contract in self:
            if contract.oa_input_ids or contract.farmer_contract_offtake_ids:
                raise UserError(
                    _(
                        f"Contract {contract.display_name} Warning"
                        " - You can't cancel a contract with input orders"
                        " or off-take order. Either cancel those orders or"
                        " close the contract."
                    )
                )
    ...
    def _cron_close_expired_contracts(self):
        for company in (
            ...
            .sudo()
            .search([("close_expired_contracts", "=", True)])
        ):
            expired_contracts = self.sudo().search(
                [("company_id", "=", company.id), ("expired_contract", "=", True)]
            )
