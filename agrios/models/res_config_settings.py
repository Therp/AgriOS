...
class ResConfigSettings(models.TransientModel):
    ...
    group_seed_varieties = fields.Boolean(
        "Seed Varieties", implied_group="agrios.group_seed_varieties"
    )
    group_plot_management = fields.Boolean(
        "Plot Management", implied_group="agrios.group_plot_management"
    )
    ...
    group_plot_maps = fields.Boolean(
        "Plot On Maps", implied_group="agrios.group_plot_maps"
    )
    ...
    land_area_uom = fields.Many2one(
        ...
        default=lambda self: self.env.ref("agrios.area_1", raise_if_not_found=False),
    )
    ...
    def _check_land_area_uom(self):
        for record in self:
            if record.land_area_uom.id == (
                ...
                or self.env.ref("agrios.area_1").id
            ):
                continue

            farmers_count = self.env["res.partner"].search_count(
                [("is_farmer", "=", True)]
            )
            if farmers_count > 0:
