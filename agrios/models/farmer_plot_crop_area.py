...
class FarmerPlotCropArea(models.Model):
    ...
    annual_estimated_yield = fields.Float(
        "Annual Estimated Yield (KG)", compute="_compute_annual_estimated_yield"
    )
        ...
        for record in self:
            if record.acreage > record.plot_size:
                raise ValidationError(
                    _(
                        "The acreage of the subplot cannot be greater"
                        " than the acreage of the linked plot."
                    )
                )
        ...
        for record in self:
            if record.product_type == "tree_crop":
                if not record.number_of_trees or not record.year_of_plantation:
                    raise ValidationError(
                        _(
                            f"For tree crops in plot {record.plot_id.name},"
                            " both the 'Number of Trees' and 'Year of Plantation'"
                            " must be provided."
                        )
                    )
                if record.year_of_plantation < 1800:
                    raise ValidationError(
                        _(
                            f"Error in plot {record.plot_id.name}."
                            " Year of Plantation cannot be less than 1800."
                        )
                    )
                if record.year_of_plantation > current_year:
                    raise ValidationError(
                        _(
                            f"Error in plot {record.plot_id.name}."
                            " Year of Plantation cannot be in the future."
                        )
                    )

    def _inverse_farmer_harvest_products(self):
        for record in self:
