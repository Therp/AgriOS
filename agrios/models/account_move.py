    ...
    def _onchange_payment_in_kind(self):
        if self.payment_in_kind:
            if not self.journal_id.payment_in_kind:                
                ...
                payment_in_kind_journals = self.env['account.journal'].search([('payment_in_kind','=',True),('type','=','sale'),('company_id','=',company.id)])
                if not payment_in_kind_journals:
                    self.payment_in_kind = False
                    warning = {
                        'title': _('Warning!'),
                        'message': _("No journals defined as a 'payment in kind' journal"),
                    }
