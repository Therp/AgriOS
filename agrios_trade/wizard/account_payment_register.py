...
class AccountPaymentRegister(models.TransientModel):
    ...
    payment_in_kind_invoice_ids = fields.Many2many(
        "account.move", compute="_compute_payment_in_kind"
    )
    payment_in_kind_full_amount = fields.Monetary(
        compute="_compute_payment_in_kind",
    )
    payment_in_kind_amount = fields.Monetary(
        "Payment In Kind Deduct Amount", compute="_compute_payment_in_kind"
    )
    apply_payment_in_kind = fields.Boolean(
        "Deduct Payment In Kind Amount",
        compute="_compute_apply_payment_in_kind",
        precompute=True,
        store=True,
    )
        ...
        for wizard in self:
            if (
                ...
                wizard.payment_in_kind_invoice_ids = (
                    wizard.payment_in_kind_full_amount
                ) = wizard.payment_in_kind_amount = False
        ...
        if self.apply_payment_in_kind:
            ...
            for bill in self.line_ids.mapped("move_id"):
                for invoice in self.payment_in_kind_invoice_ids:
                    invoice_bill_amount_to_reconcile = min(
                        invoice.amount_residual, bill.amount_residual
                    )
                    if invoice_bill_amount_to_reconcile:
                        inv_debit_line = invoice.line_ids.filtered(
                            lambda invl: invl.debit
                        ).sorted("debit")
                        bill_credit_line = bill.line_ids.filtered(
                            lambda invl: invl.credit
                        ).sorted("credit")
                        if inv_debit_line and bill_credit_line:
                            ...
                            line_ids = [
                                (0, 0, line) for line in [credit_line, debit_line]
                            ]
                            reconcile_entry = account_moves_env.create(
                                {
                                    "move_type": "entry",
                                    "ref": _("Payment In Kind Reconciliation")
                            ...
                            (
                                inv_debit_line
                                + reconcile_entry.line_ids.filtered(
                                    lambda rec_line: rec_line.credit
                                )
                            ).reconcile()
                            (
                                bill_credit_line
                                + reconcile_entry.line_ids.filtered(
                                    lambda rec_line: rec_line.debit
                                )
                            ).reconcile()
        return payments
